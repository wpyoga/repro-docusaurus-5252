...
async function start(siteDir, cliOptions) {
    ...
    ['addDir', 'unlinkDir'].forEach((event) => fsWatcher.on(event, reload));
    fsWatcher.on('add', (path, details) => { console.log(new Date().toJSON(), 'add event', path, details); reload(); });
    fsWatcher.on('change', (path, details) => { console.log(new Date().toJSON(), 'change event', path, details); reload(); });
    fsWatcher.on('unlink', (path) => { console.log(new Date().toJSON(), 'unlink event', path); reload(); });

    fsWatcher.on('raw', (event, path, details) => console.log(new Date().toJSON(), 'raw event', event, path, details));
